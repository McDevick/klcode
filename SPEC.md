...
### 4.6 开发过程日志（AGENT_LOG）

开发期间必须实时维护根目录 `AGENT_LOG.md`，规则如下：

- 每个 task 开工时先新增一条“进行中”记录。
- 任务执行全程按事件记录，不得在任务全部完成后统一补写。
- 每条记录至少包含：时间戳、task 编号、触发的 Superpowers 技能、关键 prompt/context、subagent 输出或 commit hash、人工干预、教训。
- 每个 task 完成时补充验证命令输出、评审结论和遗留事项。
- 若发现实现越界或启动未授权功能，必须立即停止并在 `AGENT_LOG.md` 记录原因。
- `AGENT_LOG.md` 属于最终交付物，任何实现任务不得绕过该日志。
